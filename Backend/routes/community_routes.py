...
from services.community_service import get_challenges_by_creator 

@router.get("/feed/user/{user_id}", response_model=List[FeedPostResponse])
async def view_user_feed(user_id: str):
    try:
        return await get_user_posts(user_id)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@router.get("/mentorship/active", response_model=List[MentorshipSessionResponse])
async def view_active_mentorships():
    try:
        return await get_active_methorships()
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@router.get("/challenges/creator/{creator_id}", response_model=List[ChallengeResponse])
async def view_creator_challenges(creator_id: str):
    """Frontend Action: Challenge poster views all submissions on 'my-created-challenges' page."""
    try:
        return await get_challenges_by_creator(creator_id)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
